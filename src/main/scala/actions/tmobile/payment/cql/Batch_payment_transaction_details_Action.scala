...
class Batch_payment_transaction_details_Action(cassandra: Cassandra, simConfig: SimConfig) extends BaseAction(cassandra, simConfig) {

  private val tableName = "batch_payment_transaction_details"

  createKeyspace
  createTable()

  private val writebatch_payment_transaction_detailsQuery: Insert = QueryBuilder.insertInto(keyspace, tableName)
  ...
  def writeRecords: ChainBuilder = {
    ...
    val preparedStatement = session.prepare(writebatch_payment_transaction_detailsQuery)
  ...
  val readbatch_payment_transaction_detailsQuery = QueryBuilder.select("dps_payment_id", "tmotoken", "account_number", "batch_id", "account_name", "acquirer_response_code")
    .from(keyspace, tableName)
    .limit(10)

  def readRecords:  ChainBuilder = {

    val preparedStatement = session.prepare(readbatch_payment_transaction_detailsQuery)
    ...
    runQueries(
      Array(
       s"""|CREATE TABLE IF NOT EXISTS $keyspace.$tableName (
           ...
           |zip text,
           |PRIMARY KEY (dps_payment_id, tmotoken, account_number, batch_id)
           |) WITH CLUSTERING ORDER BY (tmotoken DESC, account_number DESC, batch_id DESC);"""
          .stripMargin
        )
    )
  }
